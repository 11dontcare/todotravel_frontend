...
import { acceptInvite, acceptRecruit, rejectInvite, rejectRecruit, showParticipantsByUser } from '../../service/PlanService';
      ...
      <div className={styles.scrollContainer}>
        ...
        <ul className={styles.pendingUserList}>
          {recruit.map(planUser => (
            <li key={planUser.planParticipantId} className={styles.pendingUserItem}>
              <div className={styles.content}>
              <span>[{planUser.planTitle}]</span> 플랜에<br/>
              <span>{planUser.pendingUserNickname}</span>님이 참가 요청을 보냈습니다.
              </div>
              <div className={styles.buttonSection}>
                <button onClick={() => handleAcceptRecruitClick(planUser)} className={styles.acceptButton}>수락</button>
                <button onClick={() => handleRejectRecruitClick(planUser)} className={styles.rejectButton}>거절</button>
              </div>
            </li>
          ))}
        </ul>
        ...
        <ul className={styles.pendingUserList}>
          {invite.map(planUser => (
            <li key={planUser.planParticipantId} className={styles.pendingUserItem}>
              <div className={styles.content}>
              <span>[{planUser.planTitle}]</span> 플랜에<br/>
              <span>{planUser.planUserNickname}</span>님이 초대를 보냈습니다.
              </div>
              <div className={styles.buttonSection}>
                <button onClick={() => handleAcceptInviteClick(planUser)} className={styles.acceptButton}>수락</button>
                <button onClick={() => handleRejectInviteClick(planUser)} className={styles.rejectButton}>거절</button>
              </div>
            </li>
          ))}
        </ul>
